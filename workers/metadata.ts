...
import { syncCollectionSearch, syncPersonalSearch } from '../src/lib/server/search';
	...
	async queue(batch: MessageBatch<MetadataJob>, env: { DB: D1Database }): Promise<void> {
		...
		for (const message of batch.messages) {
			...
			try {
				...
				if (current[0].collectionId) await syncCollectionSearch(db, current[0].collectionId);
