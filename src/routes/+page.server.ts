...
export const load: PageServerLoad = async (event) => {
	...
	const recentLists = await db.select({ id: list.id, title: list.title, description: list.description, slug: list.slug, username: profile.username, updatedAt: list.updatedAt }).from(list).innerJoin(profile, eq(profile.userId, list.routeProfileId)).where(and(eq(list.visibility, 'public'), eq(list.moderationState, 'active'))).orderBy(desc(list.updatedAt)).limit(8);
