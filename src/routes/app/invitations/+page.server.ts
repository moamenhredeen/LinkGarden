...
export const actions: Actions = {
	accept: async (event) => {
		...
		await event.platform!.env.DB.batch([event.platform!.env.DB.prepare('INSERT OR IGNORE INTO list_member (list_id, user_id, created_at) VALUES (?, ?, ?)').bind(invitation.listId, user.id, Date.now()), event.platform!.env.DB.prepare("UPDATE list_invitation SET status = 'accepted', accepted_at = ?, updated_at = ? WHERE id = ? AND status = 'pending'").bind(Date.now(), Date.now(), invitation.id)]); return { accepted: true };
	},
